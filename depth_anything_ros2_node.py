...
class DepthAnythingTensorRTNode(Node):
    def __init__(self):
        ...
        # --- 线程锁，用于防止回调函数并发执行，保证推理的原子性 ---
        self.lock = threading.Lock()

        # --- 创建同步订阅者 ---
        # 使用 message_filters 来确保我们收到的图像和相机信息是时间同步的
    ...
    def image_callback(self, image_msg, info_msg):
        ...
        try:
            ...
            depth_msg.header = image_msg.header # 保持相同的时间戳和坐标系
            ...
            self.get_logger().info(f'Total processing time: {processing_time:.2f} ms')
