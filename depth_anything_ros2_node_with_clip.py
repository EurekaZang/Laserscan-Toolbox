...
class DepthAnythingTensorRTNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"Depth Params: scale={self.param_scale}, shift={self.param_shift}, min={self.min_depth}m, max={self.max_depth}m")
    ...
    def image_callback(self, image_msg, info_msg):
        ...
        try:
            ...
            model_output_map = raw_output.reshape(self.trt_model.output_shape[-2:])
            depth_resized = cv2.resize(model_output_map, (original_shape[1], original_shape[0]), interpolation=cv2.INTER_LINEAR)
            
            metric_depth = np.zeros_like(depth_resized, dtype=np.float32)
            valid_mask = depth_resized > 0.0
            
            if np.sum(valid_mask) > 0:
                denominator = self.param_shift + self.param_scale * depth_resized[valid_mask]
                denominator = np.maximum(denominator, 1e-6)
                metric_depth[valid_mask] = 1.0 / denominator
                metric_depth = np.clip(metric_depth, self.min_depth, self.max_depth)
                self.get_logger().info(f"Corrected depth - min: {metric_depth.min():.3f}m, "
                                       f"max: {metric_depth.max():.3f}m, "
                                       f"valid pixels: {np.sum(valid_mask)}")
            else:
                self.get_logger().warn("No valid pixels found in model output")
            ...
            log_message = (
                ...
                f"  3. Post-Processing:    {time_post_processing:6.2f} (resize & inverse depth)\n"
